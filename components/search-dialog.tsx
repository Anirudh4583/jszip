...
export function SearchDialog({ 
      ...
      for (const node of nodes) {
        if (node.isDirectory && node.children) {
          ...
          const textExtensions = [
            'js', 'jsx', 'ts', 'tsx', 'java', 'kt', 'py', 'rb', 'php', 'cpp', 'c', 'cs', 'go', 'rs',
            'html', 'htm', 'xml', 'css', 'scss', 'sass', 'less', 'json', 'yaml', 'yml', 'toml',
            'md', 'txt', 'log', 'ini', 'cfg', 'conf', 'properties', 'gradle', 'sql', 'sh', 'bash',
            'dockerfile', 'makefile', 'gitignore', 'readme'
          ];
          
          if (textExtensions.includes(ext) || node.name.toLowerCase().includes('readme') || 
              node.name.toLowerCase().includes('license') || node.name.toLowerCase().includes('changelog')) {
            files.push(node);
          }
        }
      }
    ...
    try {
      for (const file of textFiles) {
        try {
          ...
          if (content.startsWith('[Binary file') || content.startsWith('[Unable to read')) {
            continue;
          }
  ...
  const handleResultClick = (result: SearchResult, match: SearchResult['matches'][0]) => {
          ...
          {query && (
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <div className="flex items-center space-x-4">
                <span>{searchStats.matches} matches in {searchStats.files} files</span>
                {isSearching && (
              ...
              <Badge variant="outline">{textFiles.length} searchable files</Badge>
            </div>
          )}
            ...
            <div className="p-2 space-y-2">
              {results.length === 0 && query && !isSearching ? (
                ...
                results.map((result, resultIndex) => (
                  <div key={`${result.file.path}-${resultIndex}`} className="border rounded-lg p-3 space-y-2">
                    <div className="flex items-center space-x-2">
